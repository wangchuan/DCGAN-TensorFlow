...
tf.flags.DEFINE_integer("disp", 5, "how many iterations to display")
...
def load_data_readers(FLAGS):
    data_paths = utils.get_subdirs(FLAGS.data_path)
    data_readers = [DataReader(os.path.join(FLAGS.data_path, path), FLAGS) for path in data_paths]
    print('data random shuffling...')
